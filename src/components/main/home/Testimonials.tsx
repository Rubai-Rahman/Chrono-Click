...
const Testimonials = () => {
  const { data: story, isLoading, isError, error } = useQuery<TestimonialItem[], Error>({
    queryKey: ["testimonials"],
    queryFn: fetchTestimonials,
  });

  if (isLoading) {
    return <div className="py-16 px-4 text-center">Loading testimonials...</div>;
  }
            ...
            <CarouselItem key={item.id} className="pl-4 md:basis-1/2 lg:basis-1/3">
              <div className="p-1">
                <div className="bg-white shadow-lg rounded-lg p-6 flex flex-col items-center">
                  <div className="mb-4">
                    <Image src={item.img} alt={item.name} width={100} height={100} className="rounded-full object-cover" />
                  </div>
