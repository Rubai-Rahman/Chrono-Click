  ...
  return new Promise((resolve) => {
    setTimeout(() => {
      if (email === "test@example.com" && password === "password") {
        resolve({ user: { email, displayName: "Test User", uid: "123" }, error: null });
      } else {
  ...
  return new Promise((resolve) => {
    setTimeout(() => {
      if (email && password && name) {
        resolve({ user: { email, displayName: name, uid: "456" }, error: null });
      } else {
...
export const googleSignIn = async (): Promise<AuthResponse> => {
  ...
  return new Promise((resolve) => {
    setTimeout(() => {
      resolve({ user: { email: "google@example.com", displayName: "Google User", uid: "789" }, error: null });
    }, 1000);
  });
};
