...
import { fetchAllOrders, deleteOrder, CartProduct } from "@/api-lib/orders";
...
const ManageOrdersPageContent = () => {
  ...
  const { data: orders, isLoading, isError, error } = useQuery<OrderItem[], Error>({
    queryKey: ["allOrders"],
  ...
  const deleteOrderMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      queryClient.invalidateQueries({ queryKey: ["allOrders"] }); // Invalidate orders cache
    },
    onError: (err) => {
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <h1 className="text-2xl font-bold mb-6">Manage All Orders</h1>
                ...
                {order.cart.map((item) => (
                  <div key={item._id} className="flex items-center py-2">
                    <div className="w-16 h-16 mr-4 flex-shrink-0">
                      <Image src={item.img} alt={item.name} width={64} height={64} className="object-cover rounded" />
                    </div>
